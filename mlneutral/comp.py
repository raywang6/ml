...
#%% transform features
def transformFT(trainDF: pl.DataFrame, testDF: pl.DataFrame, features: List, save_ecdf: bool = False, outputfolder: str = None, max_category: int = 100, valid_row: int = 0.6):
    ...
    trainDF = trainDF.with_columns(
        pl.fold(
            acc=pl.lit(0),
            function=lambda acc, col: acc + col.is_null().cast(pl.Int8),
            exprs=pl.all()
        ).alias("null_count")
    )
    thres = int((1-valid_row) * len(features))
    trainDF = trainDF.filter(pl.col("null_count") <= thres).drop("null_count")
    if testDF.height > 0:
