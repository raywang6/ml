...
def transformFT(
    ...
    # --- Step 4: Fit StandardScaler on training data (continuous features only) ---
    # Fill nulls with 0 for scaling (or use median - your choice)
    ...
    # Update trainDF with scaled values
    for i, col in enumerate(cont_feats):
        # Preserve original nulls
        null_mask = trainDF[col].is_null()
        new_vals = scaled_train[:, i]
        new_vals = np.where(null_mask.to_numpy(), np.nan, new_vals)
    ...
    if save_scaler and outputfolder:
        ...
        print(f"[transformFT] Saved scaler to {outputfolder}/X_scaler.pkl")
...
def process_test_data(
    ...
    feature_info_file: str = None,
    max_category: int = 100,
) -> pl.DataFrame:
    """
    处理测试数据，步骤包括：
      1. 清理无穷值
      2. 使用训练时保存的StandardScaler对连续特征进行标准化（合并前）
      3. 根据训练时保存的合并组（merge_groups），将高度相关的特征取均值合并
    ...
    参数:
      ...
      feature_info_file: 存储特征信息（cont_feats, cate_feats）的pickle文件路径（可选）
      max_category: 用于识别分类特征的阈值（如果没有feature_info_file）
    ...
    if feature_info_file and os.path.exists(feature_info_file):
        ...
        # 根据unique值数量判断
        cate_feats = [f for f in allfeatures if f in new_test.columns and new_test[f].n_unique() <= max_category]
        cont_feats = [f for f in allfeatures if f in new_test.columns and f not in cate_feats]
    
    # --- 第三步：使用StandardScaler进行标准化（合并前） ---
    try:
