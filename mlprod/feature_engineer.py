...
def generate_targets(
    ...
    if use_sw:
        data = data.with_columns(
            pl_compute_sample_weights(f"ret_T{j}").alias(f"sw_ret_T{j}") for j in horizons
        )
    #transform
    for j in horizons:
        col = f"ret_T{j}"
        ...
        if kurtosis(dropna_col) > 5:    
            ...
            if save_ecdf and outputfolder is not None:
                with open(f'{outputfolder}/{col}.pkl', 'wb') as f:
                    pickle.dump(ecdf, f)
    data = data.with_columns([
        pl.col(f"ret_T{j}").cast(pl.Float64).replace({np.inf: None, -np.inf: None, np.nan: None}).qcut([0.333, 0.667], labels = ['neg', 'neu', 'pos']).alias(f"cat_ret_T{j}")
        for j in horizons
    ])
    return data
